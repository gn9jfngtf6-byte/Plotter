...
function buildPanelInputs(containerId, labels, prefix) {
  ...
  labels.forEach((lbl, i) => {
    ...
    row.innerHTML =
      ...
      `<input type="number" id="${prefix}-x${i}" step="any" inputmode="decimal" style="width:52px;" placeholder="—">` +
      `<span style="color:var(--text-muted);">y=</span>` +
      `<input type="number" id="${prefix}-y${i}" step="any" inputmode="decimal" style="width:52px;" placeholder="—">`;
...
function showExpLoesungsweg() {
  ...
  if (p1OnY || p2OnY) {
    ...
    html.push(
      ...
      lwLine(''),
      sHead('▶ Schritt 2 — Basis b mit Logarithmus'),
      ...
      lwLine(`${xBH} · ln(b) = ln(${ratioH})`, true),
      lwLine(`ln(b) = ${mf(`ln(${ratioH})`, xBH)}`, true),
      lwLine(`<b>b = e<sup>${mf(`ln(${ratioH})`, xBH)}</sup> = ${ratioH}<sup>${mf('1', xBAbsH)}</sup> = ${bH}</b>`, true),
      lwLine(''),
      sHead('▶ Ergebnis'),
